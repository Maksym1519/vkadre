...
export const locationsPageInfo = createAsyncThunk<
  LocationsPageData,
  undefined | string,
  { rejectValue: string }
>(
  ...
  async function (location, { rejectWithValue }) {
    const response = await axios.get(
      `https://vkadrestrapi.onrender.com/api/main-locations?populate=*&filters[location][$contains]=${
        location !== "Все" ? location : ""
      }`
    );
